...
class GDeepAD(metaclass=ABCMeta):
    """
    ...
    def fit(self, X, y=None):
        ...
        for _ in range(self.n_ensemble):
            self.train_loader, self.net = self.training_prepare(self.train_data,
                                                                                y=self.train_label)
            self._training()
    ...
    def _training(self):
        ...
        optimizer = torch.optim.Adam([
            {'params':[param for name,param in self.net.named_parameters() if name!='radius'], 'lr':0.001 },
            {'params':self.net.radius, 'lr':0.01 },
                            ])
        ...
        for i in range(self.epochs):
            ...
            for batch_x in self.train_loader:
                loss = self.training_forward(batch_x, self.net,flag)
                self.net.zero_grad()
                loss.backward()
                optimizer.step()
    ...
    def _inference(self):
        ...
        with torch.no_grad():
            ...
            for batch_x in _iter_:
                batch_z, s = self.inference_forward(batch_x, self.net)
