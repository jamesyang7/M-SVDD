...
from nets.svdd_net_audio import FusionNet_audio as FusionNet
...
train_data = CollisionLoader_new(train_imu_path,train_audio_path,augment =True,mask=True)
val_data   = CollisionLoader_new(test_imu_path,test_audio_path,augment = True,mask=True)
...
val_dataloader   = DataLoader(val_data, batchsize, shuffle=True, num_workers=workers, drop_last=False)
...
for epoch in range(Epoch):
    ...
    for i, data in enumerate(train_dataloader, 0):
        ...
        reconstruction_loss = reconstruction_loss_fn(audio_recons, reconstructed_audio)
        total_reconstruction_loss += reconstruction_loss.item()
        reconstruction_losses.append(reconstruction_loss.item())

        total_loss = 0.3*reconstruction_loss+svdd_loss
        total_loss.backward()
        optimizer.step()
